...
export default class CompanyUpdate extends Vue {
    ...
    if (this.company.id) {
      this.companyService()
        ...
        .then(param => {
          this.isSaving = false;
          this.$router.go(-1);
          const message = this.$t('jhiStarterApp.company.updated', { param: param.id });
      ...
      this.companyService()
        ...
        .then(param => {
          this.isSaving = false;
          this.$router.go(-1);
          const message = this.$t('jhiStarterApp.company.created', { param: param.id });
  ...
  public retrieveCompany(companyId): void {
    this.companyService()
      .find(companyId)
      .then(res => {
        res.settleTime = new Date(res.settleTime);
        this.company = res;
      })
      .catch(error => {
        this.alertService().showHttpError(this, error.response);
